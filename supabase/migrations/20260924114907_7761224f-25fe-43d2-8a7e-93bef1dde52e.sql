...
BEGIN
  ...
  IF _role = 'admin'::public.app_role THEN
    RAISE EXCEPTION 'Não é permitido auto-atribuir o papel admin' USING ERRCODE = '42501';
  END IF;
  ...
  IF _existing_role = _role THEN
    RETURN;
  END IF;

  IF _existing_role IS NOT NULL THEN
    RAISE EXCEPTION 'Usuário já possui um papel diferente definido' USING ERRCODE = '22023';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.assign_initial_role(public.app_role) TO authenticated, service_role;